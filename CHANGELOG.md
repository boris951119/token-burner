...
- **冻结原因前置展示**:pipeline `module_done` 事件携带截尾原因;
  工作台模块卡片对 FROZEN 直接显示「原因摘要 + 建议动作」(查看
  fix_history / 调高修复轮 / 换更强开发模型),日志同步输出——不再需要
  翻 validation.md。safe 模式下冻结经反馈轮发生,原因经 awaiting 阶段
  的 message 字段同样可达(全量 1064→1071 绿)。

- **交付目录可配置**:① 设置页新增「📁 交付目录」,保存即热切换
  (`POST /api/settings/projects-root`,进行中任务不受影响);
  ② `POST /api/tasks|run` 新增 `project_dirname`(自定义目录名,
  清洗 + 时间戳,同秒同名自动加随机后缀;缺省自动命名,行为兼容)。

- **首启向导(三步)**:首次打开(无连接且未跳过过)自动弹出——
  ①选供应商模板卡 ②贴 Key + 自动发现 + 探针验证 ③完成。
  「跳过向导」记忆于本地;.env 老用户不受影响(有连接即不弹)。

- 测试:新增 `test_c3_final.py` 7 项(目录名清洗/碰撞后缀/根目录热切换
  守卫/module_done message 透传);全量回归 **1064 → 1071 passed /
  7 skipped / 0 failed**。
...
> + V2.9 契约注入 + M15-8 测试重写）:**模块 37/52 = 71%**,任务 10/10,
> 零超支——v1.0 Done 定义(全量 ≥60% + 预算超支 <10%)全部达成。

- **三轮演进**:R1 47% → R2 57%(+10pt,M15-8)→ R3 71%(+14pt,触发提前
...
> full_r2:28/49=57%(+10pt,M15-8 生效),仍差 60% 门槛 3pt → 预案内第 2 轮
> 回炉(最后一轮)。R2 冻结 21 分类:断言 11(样本仍为 1 failed/25 passed
> 过度规格形态)、收集 3、接口门禁 3、其他 3、危险拦截 1(hard 类按设计)。

- **M15-8 触发提前**:断言类失败的重写测试从第 3 修复轮提前到第 2 轮
  (R2 取证:M15-8 在 analytics_engine 正常触发并重写,但第 3 轮才出手
  已耗 2 轮预算,重写后余量不足)。
- **dev 阵容**:deepseek-v4-flash(76 tok/s)→ **deepseek-v4-pro**
  (44 tok/s,端点内最强语义模型)——语义残差类冻结的最后杠杆,以速度
  换最终通过率(用户既定方针:最先进模型亲测)。
- 测试同步更新(轮 2 触发);全量回归 **1035 passed / 0 failed**。
- R2 归档 `logs/bench_v1/full_r2/`(逐任务:T3 3/4、T7 3/6 +2、T9 4/5,
  T4 0/6 为最差)。


## v1.0 · V3 回炉轮（2026-09-06）：M15-8 修复轮测试重写 + 契约私有化防护

> V3 全量基准第一轮（10 任务）:任务级 10/10,模块 26/55=47%,未达 60% →
> 回炉。29 个 FROZEN 分类:断言失败 16、接口门禁 extra 7、收集错误 4、
> 其他 2、**危险拦截 0**（V2.8 全量规模零误杀）。

- **M15-8a 契约私有化防护**（scorer 类）:全量取证 T1/passmint_scorer——
  测试 import `DefaultScoringRule`,代码实现为 `_DefaultScoringRule`
  （提示词"契约外符号请私有化"被过度应用）。M15-7 契约注入段补
  「契约导出符号禁止加下划线前缀,_X 不满足 X 的契约」。

- **M15-8b 修复轮测试重写**（history 类,16/29 断言失败的主解药）:
  `_drive` 修复策略升级——exit_code=1 断言类失败连续 3 轮 → 疑似测试
  过度规格,paid_pilot3/history 即 17/18 通过、1 个平台相关权限断言拖死
  整模块 → 第 3 修复轮重写测试（保持契约覆盖、断言对齐实际行为、禁平台
  相关断言）,轮 4-5 回到修代码。exit_code=2（收集错误）不触发,维持
  修代码路径。

- **写测试模板**:增平台相关断言禁令（权限位/换行符/路径分隔符）。

- 测试:新增 `test_m15_8.py` 5 项(轮 3 重写触发/exit_code=2 不触发/
  无失败不触发/私有化警告注入/模板禁令);全量回归 **1030 → 1035 passed /
  7 skipped / 0 failed**。基准数据 `logs/bench_v1/full/` 归档。
